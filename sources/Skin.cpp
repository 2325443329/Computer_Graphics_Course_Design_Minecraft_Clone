...
void Skin::switch_skin(bool next) {
	if (next) {
		// 切换到下一个皮肤
		current_skin_index = (current_skin_index + 1) % Texture::player_skins.size();
	}
	else {
		// 切换到上一个皮肤
		current_skin_index = (current_skin_index - 1 + Texture::player_skins.size()) % Texture::player_skins.size();
	}
}
...
void Skin::draw(const Camera& camera, const std::vector<const Light*>& lights, const Plane& clipping_plane) const {
	Shader::entity.bind();
	// 绑定当前皮肤的纹理
	Texture::player_skins[current_skin_index].bind(0);
